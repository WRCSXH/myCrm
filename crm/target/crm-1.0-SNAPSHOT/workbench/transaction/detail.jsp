...
<script type="text/javascript">
	...
	function changeStage(stage,i) {
		if (confirm("您确定要改变交易阶段吗，这种行为将永久产生交易历史！")){
			// alert(stage);
			// alert(i);
			$.ajax({
				...
				data:{
					"id":"${tran.id}",
					"stage":stage,
					"money":"${tran.money}", // 用于生成交易历史
					"expectedDate":"${tran.expectedDate}" // 用于生成交易历史
				},
				dataType:"json",
				success:function(result){
					// result{"isSuccess":true/false,"tran":交易对象}
					if (result.isSuccess){
						// 改变交易阶段成功后

						// 在详细信息页上进行局部刷新，刷新阶段、可能性、修改人、修改时间
						$("#stage").html(result.tran.stage);
						$("#possibility").html(result.tran.possibility);
						$("#editBy").html(result.tran.editBy+"&nbsp;&nbsp;");
						$("#editTime").html(result.tran.editTime);
	...
	function changeIcon(index) {
		...
		// 当前阶段的下标
		var index = index;
		// 前面正常阶段和后面丢失阶段的分界点下标，其实是第一个丢失阶段的下标
		var point = "<%=point%>";
		...
		*/
		...
		// 如果当前阶段的可能性为0，前面七个一定是黑圈，后面两个中一个黑叉，一个红叉
		...
		// 如果当前阶段的可能性不为0，前面七个有可能是绿圈、绿标、黑圈，后面两个一定是黑叉
		} else {
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			// 准备当前阶段的可能性
			String currentPossibility = stagePossibilityMap.get(currentStage);

			// 判断当前阶段
			// 如果当前阶段的可能性为0，前七个一定是黑圈，后两个中一个是黑叉，一个是红叉
			if ("0".equals(currentPossibility)){
				...
				for (int i = 0;i < dicValueList.size();i++){
					...
					// 再根据阶段取得对应的可能性
					String listPossibility = stagePossibilityMap.get(listStage);

					// 如果遍历出来的阶段可能性为0，说明是后两个，一个是黑叉，一个是红叉
		...
		%>
							<span id="<%=i%>"
								  onclick="changeStage('<%=listStage%>','<%=i%>')"
		...
		%>
							...
							-----------
		<%
						// 如果遍历出来的阶段可能性不为0，说明是前七个，有可能是绿圈、绿标、黑圈
						} else {
